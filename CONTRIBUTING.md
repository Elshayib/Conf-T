...
1. Create a file in `conf_t/lessons/<platform_slug>.json`.
2. Follow the [Lesson JSON Schema](README.md#-adding-custom-lessons) documented in the README.
3. Ensure every `expected` field is a **valid Python regex** (test it with `re.compile()`).
4. Make sure all `id` values are unique across all lesson files.

---

## Adding a New Platform

1. Create a lesson file: `conf_t/lessons/<platform>.json`
...
4. Document the new platform's case rule in [`.agents/AGENTS.md`](.agents/AGENTS.md).
